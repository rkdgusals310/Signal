...
const ArticleComment = ({ articleId }) => {
  ...
  const fetchComments = async (reset = false) => {
    ...
    try {
      const url = cursorId
        ? `/api/common/article/${articleId}/comment?cursorId=${cursorId}&size=10`
        : `/api/common/article/${articleId}/comment?size=10`;
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`/api/common/article/${articleId}/comment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          articleId, // `articleId`를 요청 본문에 추가
          contents: newComment,
        }),
      });
  ...
  const confirmEditComment = async () => {
    ...
    if (confirmEdit) {
      try {
        const response = await fetch(`/api/common/article/${articleId}/comment/${editingCommentId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      const response = await fetch(
        `/api/common/article/${articleId}/comment/${commentId}`,
        {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
        }
